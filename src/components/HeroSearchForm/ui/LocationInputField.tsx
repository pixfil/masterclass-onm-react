...
import { useLocalites } from '@/hooks/useLocalites'
...
export const LocationInputField: FC<Props> = ({
  ...
  description = T['HeroSearchForm']['Location, city, or property name'] || 'Ville, quartier ou nom de bien',
  className = 'flex-1',
  inputName = 'location',
  ...
  // Hook pour récupérer les localités depuis Supabase
  const { localites, loading } = useLocalites(searchTerm)
  ...
  // Utiliser les locations passées en props ou fallback sur le hook
  const supabaseLocalites: Suggest[] = locations.length > 0 
    ...
    : localites.map(localite => ({
        id: localite.id,
        name: `${localite.name} (${localite.count} bien${localite.count > 1 ? 's' : ''})`,
        icon: Location01Icon
      }))
  
  const isShowInitSuggests = !searchTerm
  const suggestsToShow = isShowInitSuggests ? supabaseLocalites.slice(0, 5) : supabaseLocalites
  const showLoading = locations.length > 0 ? isLoading : loading
  return (
    <div
      ...
      >
        ...
        <Headless.Transition show={showPopover} unmount={false}>
          <div className={clsx(styles.panel.base, styles.panel[fieldStyle])}>
            ...
            {!isShowInitSuggests && showLoading && (
              <p className="mt-2 mb-3 px-4 text-xs/6 font-normal text-neutral-600 sm:mt-0 sm:px-8 dark:text-neutral-400">
                Recherche en cours...
              </p>
            )}
            {(isShowInitSuggests || showLoading) && <Divider className="opacity-50" />}
