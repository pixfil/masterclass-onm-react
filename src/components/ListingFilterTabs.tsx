...
const ListingFilterTabs = ({
  ...
  // Fonction pour mapper les noms de filtres aux paramètres URL
  const getUrlParamName = (filterName: string): string => {
    switch (filterName) {
      case 'transaction':
        return 'transaction'
      case 'location':
  ...
  // Adapter les filterOptions avec les searchParams pour pré-sélectionner les filtres
  const adaptedFilterOptions = filterOptions.map((filterOption) => {
    if (!filterOption) return filterOption

    if (filterOption.tabUIType === 'checkbox') {
      const urlParamName = getUrlParamName(filterOption.name)
      const selectedValue = searchParams[urlParamName] as string
      
      if (!selectedValue) return filterOption
      ...
      return {
        ...
        options: filterOption.options?.map((option) => {
          ...
          if (filterOption.name === 'transaction') {
            // Correspondance pour type de transaction
            isSelected = selectedValue === option.value
          } else if (filterOption.name === 'location') {
            // Pour la localisation, on compare avec la partie avant les parenthèses
            const locationName = option.name.split(' (')[0]
            isSelected = selectedValue.includes(locationName) || locationName.includes(selectedValue)
          } else if (filterOption.name === 'property-type') {
            // Pour le type de bien
            isSelected = selectedValue === option.value
          } else {
  ...
  // Calculer le nombre total de filtres sélectionnés
  const totalSelectedFilters = adaptedFilterOptions.reduce((total, filterOption) => {
    if (filterOption?.tabUIType === 'checkbox') {
      return total + ((filterOption as CheckboxFilter).options?.filter(option => option.defaultChecked)?.length || 0)
    }
    return total
  }, 0)
            ...
            >
              ...
              <div className="hidden-scrollbar grow overflow-y-auto text-start">
                <div className="divide-y divide-neutral-200 px-4 sm:px-8 dark:divide-neutral-800">
                  {adaptedFilterOptions.map((filterOption, index) =>
                    filterOption ? (
  ...
  if (!adaptedFilterOptions || adaptedFilterOptions.length === 0) {
