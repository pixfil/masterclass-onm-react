-- Script SQL corrigé pour créer la table des experts CEPROF
...
DROP POLICY IF EXISTS "Permettre lecture experts" ON ceprof_experts;
DROP POLICY IF EXISTS "Permettre modifications admin experts" ON ceprof_experts;

-- Politique pour permettre la lecture à tous
CREATE POLICY "Permettre lecture experts" ON ceprof_experts
    FOR SELECT USING (true);

-- Politique pour permettre les modifications aux administrateurs seulement
CREATE POLICY "Permettre modifications admin experts" ON ceprof_experts
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM auth.users 
            WHERE auth.users.id = auth.uid()
            AND auth.users.email = 'admin@masterclass-onm.com'
        )
    );
